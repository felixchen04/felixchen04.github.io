...
请修改已有分组，不要在文件末尾再次添加同名 `[profile]`。正文是纯文本，不是完整 Markdown：简介、作者、论文概要、摘要及经历/笔记描述支持 `**加粗**`；简介和摘要还支持空行分段。正文链接可写成 `[Lei Wu](https://leiwu0.github.io/)`；联系方式及论文链接仍使用专用 `url` 字段。HTML 标签会按普通文字显示。
